 ...
 *
 * <p>Paths are relative. For an annotated class the base URI is the application path. For an
 ...
 * }</pre>
 *
 * <p>In the above, if the application path is {@code catalogue} and the application is deployed at
 ...
 * requests for <code>http://example.com/catalogue/widgets/<i>nnn</i></code> (where <code><i>nnn</i>
 * </code> is some value) will be handled by the {@code getWidget} method. The same would apply if
 * the value of either {@code @Path} annotation started with '/'.
 *
 * <p>Classes and methods may also be annotated with {@link Consumes} and {@link Produces} to filter
...
public @interface Path {
   ...
   *
   * <p>Embedded template parameters are allowed and are of the form:
   *
   ...
   * </pre>
   *
   * <p>See {@link <a href="http://tools.ietf.org/html/rfc5234">RFC 5234</a>} for a description of
   ...
   * regex} should be escaped by the author according to the rules of {@link <a
   * href="http://tools.ietf.org/html/rfc3986#section-3.3">RFC 3986 section 3.3</a>}. Caution is
   * recommended in the use of {@code regex}, incorrect use can lead to a template parameter
   * matching unexpected URI paths. See {@link <a
   * href="http://download.oracle.com/javase/6/docs/api/java/util/regex/Pattern.html">Pattern</a>}
   ...
   *
   * <p>The literal part of the supplied value (those characters that are not part of a template
   * parameter) is automatically percent encoded to conform to the {@code path} production of {@link
   * <a href="http://tools.ietf.org/html/rfc3986#section-3.3">RFC 3986 section 3.3</a>}. Note that
