...
public class JavaStandardRestfulRequest extends BaseRestfulRequest {
  ...
  // Immutable basics from BaseRestfulRequest ctor args
  private final String method;

  // Mutable request state prior to send()
  private final Map<String, List<String>> queryParams = new LinkedHashMap<>();
  private final Map<String, String> headers = new LinkedHashMap<>();

  // Client & send-time config
  private final HttpClient httpClient;
  private volatile int timeoutMillis = 0; // 0 = not set

  // In-flight handle to support abort()
  ...
  public RestfulRequest timeout(int timeout) {
    // store; applied per-request on the HttpRequest builder
  ...
  private static boolean allowsRequestBody(String method) {
    // Looser predicate; some servers tolerate bodies on GET/DELETE
    return true;
  }
  ...
  private static byte[] buildMultipartBody(MultipartForm form, String boundary) throws IOException {
    ...
    // File parts (provided as bytes in your MultipartForm)
